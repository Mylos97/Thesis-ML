...
def main(args) -> None:
    ...
    if args.model == 'bvae':
        ...
        """
        data, in_dim, out_dim = load_autoencoder_data(path=get_relative_path('retrain-1.txt', 'Data/splits/imdb/training'), retrain_path=args.retrain, device=device, num_ops=args.operators, num_platfs=args.platforms)
        test_data, _, _ = load_autoencoder_data(path=get_relative_path('retrain-1.txt', 'Data/splits/imdb/training'), retrain_path='', device=device, num_ops=args.operators, num_platfs=args.platforms)
        val_data, _, _ = load_autoencoder_data(path=get_relative_path('retrain-1.txt', 'Data/splits/imdb/training'), retrain_path='', device=device, num_ops=args.operators, num_platfs=args.platforms)
        model_class = BVAE
