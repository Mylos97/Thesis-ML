...
def hyperparameterBO(model_class: nn.Module,  data, in_dim:int, out_dim:int , loss_function:nn.MSELoss | nn.BCELoss, device: torch.device):
    BATCH_SIZE = 64
    train_loader, val_loader, test_loader = torch.utils.data.random_split(data, [0.8, 0.1, 0.1])

    def train_evaluate(params):
        model = train(model_class=model_class, data_loader=train_loader, 
    ...
    best_model = train(model_class=model_class, data_loader=combined_train_valid_loader, in_dim=in_dim, out_dim=out_dim, loss_function=loss_function, device=device, parameters=best_arm)
